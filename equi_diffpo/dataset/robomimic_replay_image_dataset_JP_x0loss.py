...
print('Using RobomimicReplayImageDataset with JP rotation representation.')


class RobomimicReplayImageDataset_JP_x0loss(BaseImageDataset):
    def __init__(self,
                 shape_meta: dict,
                 dataset_path: str,
                 horizon=1,
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        torch_data = {
            ...
            'x0loss': torch.from_numpy(data['x0loss'].astype(np.float32))
...
def _convert_robomimic_to_replay(store, shape_meta, dataset_path, abs_action, rotation_transformer,
    ...
    with h5py.File(dataset_path) as file:
        ...
        # X0LOSSPLUGIN
        # save lowdim data
        for key in tqdm(lowdim_keys + ['action'] + ['x0loss'], desc="Loading lowdim data"):
            data_key = 'obs/' + key
            if key == 'action':
                data_key = 'actions'
            if key == 'x0loss':
                data_key = 'x0loss/eePose'
            this_data = list()
            for i in range(n_demo):
                demo = demos[f'demo_{i}']
            ...
            if key == 'action':
                ...
                assert this_data.shape == (n_steps, ) + tuple(shape_meta['action']['shape'])  # eePose
            else:
...
def print_dict_shapes(d: dict, indent: int = 0):
    """
    Recursively print a nested dict:
      - If a value is a dict, recurse into it.
      - Else if it’s ndarray/tensor/list, print its shape.
      - Otherwise print its type name.
    """

    def _get_shape(val):
        # NumPy array or anything with .shape
    ...
    for key, val in d.items():
        ...
        if isinstance(val, dict):
            print(f"{prefix}{key}:")
            print_dict_shapes(val, indent + 1)
        else:
...
if __name__ == "__main__":
    ...
    shape_meta = {
        "obs": {
            ...
            "robot0_eef_pos": {
                "shape": [3],
                # type defaults to "low_dim" if not specified
            },
            "robot0_eef_quat": {
                "shape": [4],
            },
